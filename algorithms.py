...
class NRO:
    def __init__(self, k=0, gamma=1.0, p_norm=2, regions='S', n_samples=200,
    ...
    def fit_sample(self, X, y):
        ...
        for i in range(len(minority_points)):
            ...
            sorted_distances_k = np.argsort(distances[i])[self.k]
            radius = distances[i, sorted_distances_k] - 0.01
            n_majority_points_within_radius = self.k
            radii[i] = radius
...
class MultiClassNRO:
    def __init__(self, method='sampling'):

        assert method in ['sampling']
        self.method = method
    ...
    def fit_sample(self, X, y):
        ...
        for i in range(1, len(classes)):
            ...
            nro = NRO(minority_class=current_class, n=n)
